    ...
    return (
        <Header className="header">
            <Breadcrumb className="header__pathLine pathLine" separator={""}>
                <Breadcrumb.Item className="pathLine__item">
                    Главная
                </Breadcrumb.Item>
            </Breadcrumb>
            <div
                className={isCollapsed ? "content__wrapper content__wrapper--collapsed" : "content__wrapper header__wrapper--uncollapsed"}>
                <div className="header__wrapper">
                    <h1 className={'header__title'}>{headerTitle1}</h1>
                    <h2 className={'header__title'}>{headerTitle2}</h2>
                </div>
                { isBigMobile ? <Button {...props}>Настройки</Button> : <div className="settings__outline"> <Button icon={<DynamicIcon
                    type="SettingOutlined"
                    props={{style:{ color: '#262626', fontSize: '14px'}}}
                    style={{ color: '#262626', fontSize: '14px'}}
                    className="settings__icon"/> } {...props}/></div>}
